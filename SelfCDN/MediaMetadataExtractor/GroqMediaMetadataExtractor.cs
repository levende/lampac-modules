...
{
    internal class GroqMediaMetadataExtractor : IMediaMetadataExtractor, IDisposable
    {
        ...
        private readonly string _modelName; //"deepseek-r1-distill-llama-70b";

        private readonly int _batchSize;
        private bool _disposed;

        public GroqMediaMetadataExtractor(string apiToken, string model, int batchSize = 20)
        {
            ArgumentNullException.ThrowIfNull(apiToken, nameof(apiToken));
        ...
        public async Task<IReadOnlyCollection<MediaMetadata>> ExtractAsync(IEnumerable<string> filePaths)
        {
            ...
            {
                var batch = list.Skip(i).Take(_batchSize);

                var batchMediaInfo = await ParseMediaInfoAsync(batch.ToList());
                result.AddRange(batchMediaInfo);

                if (i + _batchSize < list.Count)
                {
        ...
        private async Task<IEnumerable<MediaMetadata>> ParseMediaInfoAsync(IEnumerable<string> filePaths)
        {
            try
            {
                var prompt = BuildPrompt(filePaths);
            ...
            {
                throw new GroqMediaParserException($"Failed to parse media info: {ex.Message}", ex);
            }
        }

        private string BuildPrompt(IEnumerable<string> fileNames)
        {
...
=== Example input ===
```json
[""A.Working.Man.2025.1080p.WEB-RIP.H.264.mkv"",
""Krepkii_oreshek.1988.BDRip.1080p.mkv"",
""[02] S01E02 Атака титанов - Тот день(Падение Сигансины. Часть 2) [720p x264] StudioBand.mkv"",
""asda12312dasd.mp4""
]
```
...
[
{{
""FileName"":""A.Working.Man.2025.1080p.WEB-RIP.H.264.mkv"",
""Title"":""A Working Man"",
""Year"":2025
}},
{{
...
""FileName"":""[02] S01E02 Атака титанов - Тот день(Падение Сигансины. Часть 2) [720p x264] StudioBand.mkv"",
""Title"":""Крепкий Орешек"",
""SeasonNumber"":1,
""EpisodeNumber"":2,
...
";
        ...
        private async Task<string> SendApiRequestAsync(object requestBody)
        {
            ...
            {
                throw new GroqMediaParserException($"Groq API error: {response.StatusCode}");
            }
        ...
        private static string ExtractResponseContent(string responseContent)
        {
            ...
            {
                throw new GroqMediaParserException("JSON response does not contain 'choices' array or it is empty.");
            }

            var content = choices[0].GetProperty("message").GetProperty("content").GetString()
                          ?? throw new GroqMediaParserException("Content field is empty or missing.");

            var match = JsonContentRegex.Match(content);
            if (!match.Success)
            {
                throw new GroqMediaParserException("JSON content is not properly wrapped in ```json code fences or no valid JSON found.");
            }

            return match.Groups[1].Value.Trim();
        }

        private static IEnumerable<MediaMetadata> DeserializeMediaInfo(string jsonContent)
        {
            try
            {
                ...
                }) ?? throw new GroqMediaParserException("Failed to deserialize FileMediaInfo object.");
